        ...
        <div  v-if="parsedSpec.securitySchemes">
          ...
          <table style="margin-top:5px;" class="sw-table">
            ...
            <tr v-for="(scheme,key) in parsedSpec.securitySchemes" :key="key">
              ...
              <!-- If scheme is http -->
              <td v-if="scheme.type==='http' && scheme.scheme ">
                <!-- Basic or Bearer -->
                <span v-if="scheme.scheme==='basic'" >  
                    Send requests with the <code>'Authorization'</code> 
                    header that contains the word  <code>'Basic'</code> word followed by a space and a base64-encoded string username:password.
                </span>
                <span v-if="scheme.scheme==='Bearer'" >  
                    Send requests with the <code>'Authorization'</code> 
                    header that contains the word  <code>'Bearer'</code> word followed by a space and a Token String, generated by server.
                </span>
                <span></span>
              </td>  
              <!-- If scheme is oauth2 -->
